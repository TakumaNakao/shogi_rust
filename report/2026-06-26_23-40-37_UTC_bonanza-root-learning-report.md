...
### policy anchor with rerank feedback

目的:

`rerank feedback loss` は5k/500で通ったが、9k/1kではmean regretだけをわずかに悪化させた。局所的なfeedback更新が他局面に副作用を出していると判断し、baseline root分布から離れすぎないよう `policy_anchor` を追加した。
...
- `src/bin/mmto_tree_train.rs`
  ...
  - `--policy-anchor-temperature-cp`
  - `--policy-anchor-feature-source`
- `tools/run_mmto_from_dump.sh`
  - `POLICY_ANCHOR_WEIGHTS`
  - `POLICY_ANCHOR_WEIGHT`
  - `POLICY_ANCHOR_TEMPERATURE_CP`
  - `POLICY_ANCHOR_FEATURE_SOURCE`

設計:

候補集合上で、anchor modelのroot policy分布をtargetにし、現在model分布とのcross entropyを下げる。現時点では独立した追加更新ステップとして実行し、`POLICY_ANCHOR_WEIGHT` は学習率倍率として扱う。狙いは、feedbackで直す局面以外のroot選択をbaselineから大きく崩さないこと。
...
- `FONTCONFIG_NO_PKG_CONFIG=1 RUST_FONTCONFIG_DLOPEN=1 cargo check --bin mmto_tree_train`
- `FONTCONFIG_NO_PKG_CONFIG=1 RUST_FONTCONFIG_DLOPEN=1 cargo test --all-targets`
- dry-runでanchor weights読み込みとbaseline policy_anchor metric出力を確認。

#### 9k/1k feedback + policy anchor
...
`data/mmto/runs/mmto_feedback_anchor_9k_w3_a005_20260627_081012`
...
- `POLICY_ANCHOR_TEMPERATURE_CP=100`
- `POLICY_ANCHOR_FEATURE_SOURCE=move`
- `EPOCHS=3`
- `LEARNING_RATE=0.0003`
- `MAX_WEIGHT_DELTA=0.003`
- `RERANK_DEDUPE_SFEN=1`
...
| side | mean | p90 | p95 | match | bad50 | bad100 |
|---|---:|---:|---:|---:|---:|---:|
| baseline | `243.65791` | `27.491108` | `42.636803` | `44.155845%` | `3.4238487%` | `0.5903188%` |
| candidate | `243.64381` | `27.491108` | `42.636803` | `44.391972%` | `3.4238487%` | `0.5903188%` |

前回feedback-onlyとの比較:

| run | mean | p90 | p95 | match | result |
|---|---:|---:|---:|---:|---|
| baseline | `243.65791` | `27.491108` | `42.636803` | `44.155845%` | - |
| feedback only | `243.70686` | `27.491108` | `42.636803` | `44.155845%` | failed |
| feedback + policy anchor | `243.64381` | `27.491108` | `42.636803` | `44.391972%` | passed |

解釈:

policy anchorにより、9k/1kでのmean悪化が消え、matchも微増した。これは、長時間学習に向けた目的関数として初めて、5k/500と9k/1kの両splitを通った構成である。ただし改善幅は小さいため、まだ本格長時間学習ではなく、次は別seed/別subsetまたは10k級で再現確認する。

注意:

final policy_anchor top_matchは `81.67%` まで下がっており、anchorが強く効きすぎているわけではない。むしろ現状は「feedback改善を許しつつ、全体mean悪化を抑える」方向に働いている。次の感度確認では `POLICY_ANCHOR_WEIGHT=0.02` と `0.10` を比較する価値がある。

判断:

- `feedback + policy anchor` は長時間学習候補として有望。
- ただし10k以上へ進む前に、別subsetで少なくとも1回、5k/500と9k/1kの両方を通す必要がある。
- 採用条件は、mean/p90/p95/match/bad50/bad100の非悪化、できればmeanまたはmatch改善。

運用メモ:

このrunに残っていた `best.raw.binary` は容量節約のため削除した。結果は `rerank_gate.json`、`score_gate.json`、`train_stdout.log` に残っている。

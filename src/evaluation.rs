...
const LOW_CHECK_LEGAL_LIMIT: usize = 3;
const MATE_EVAL: f32 = 1_000_000.0;
...
impl SparseModel {
    ...
    pub fn predict_search_from_position(&self, pos: &shogi_lib::Position) -> f32 {
        if pos.in_check() {
            let moves = pos.legal_moves();
            if moves.is_empty() {
                return -MATE_EVAL;
            }
            if moves.len() <= LOW_CHECK_LEGAL_LIMIT {
                let mut best = -MATE_EVAL;
                for mv in moves {
                    let mut child = pos.clone();
                    child.do_move(mv);
                    best = best.max(-self.predict_from_position(&child));
                }
                return best;
            }
        }
        self.predict_from_position(pos)
    }

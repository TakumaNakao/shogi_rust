...
--- Top 10 Weights ---"
    ...
    for &index in indices.iter().rev().take(10) {
        ...
        let turn = if index < evaluation::MAX_FEATURES / 2 {
            Color::Black
        } else {
            Color::White
        };
        if let Some((king_sq, p1k, p1sq, p1hi, p1c, p2k, p2sq, p2hi, p2c)) =
            index_to_kpp_info(index)
        {
            println!("Weight: {:.6}, Index: {} (Turn: {:?})", weight, index, turn);
            println!(
                "  King (Normalized): Black at {:?} (Corresponds to White's King at {:?})",
                king_sq,
                king_sq.flip()
            );
            println!(
            ...
            println!(
                "  SFEN (Normalized): {}",
                evaluation::generate_sfen(
                    king_sq, p1k, p1sq, p1hi, p1c, p2k, p2sq, p2hi, p2c, turn
                )
            );
        } else {

...
// SFEN文字列から局面を生成する関数
fn position_from_sfen(sfen_line: &str) -> Option<Position> {
    let full_usi_command = format!("position {}", sfen_line);
    shogi_core::PartialPosition::from_usi(&full_usi_command)
        .ok()
        .map(Position::new)
}
...
fn main() -> Result<()> {
    ...
    while games_done < NUM_GAMES {
        ...
        if (games_done / 1000) > ((games_done.saturating_sub(training_batch.len())) / 1000) {

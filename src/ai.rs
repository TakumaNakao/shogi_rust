...
const NULL_MOVE_EVAL_MARGIN: f32 = 0.0;
...
impl<E: Evaluator, const HISTORY_CAPACITY: usize> ShogiAI<E, HISTORY_CAPACITY> {
    ...
    pub fn null_move_attempts(&self) -> u64 {
    ...
    pub fn alpha_beta_search(
        ...
        self.alpha_beta_search_inner(position, depth, alpha, beta, true)
    }

    fn alpha_beta_search_inner(
        &mut self,
        position: &mut Position,
        depth: u8,
        mut alpha: f32,
        ...
        if allow_null
            ...
            && beta.is_finite()
            && !position.in_check()
            && self.sennichite_detector.get_position_count(position) <= 1
        {
        ...
        for mv in sorted_moves {
            ...
            let search_result = match sennichite_status {
                ...
                SennichiteStatus::None => {
                    self.alpha_beta_search_inner(position, depth - 1, -beta, -alpha, true)
                }
            };
            self.sennichite_detector.unrecord_last_position();
            position.undo_move(mv);

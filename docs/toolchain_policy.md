...
Linux CIはformat、HalfKP-32 library test、HalfKP-64 workspace test、all-target check、
...
1. 候補toolchainを固定したCI jobで`cargo test --workspace --release --features halfkp64`を通す。
2. `cargo check --workspace --all-targets --features halfkp64`を通す。

...
SUBSET_MODE="${SUBSET_MODE:-shuffle}"
SUBSET_SEED="${SUBSET_SEED:-7201}"
SUBSET_EXCLUDE_IN_CHECK="${SUBSET_EXCLUDE_IN_CHECK:-1}"
SUBSET_MIN_LEGAL_MOVES="${SUBSET_MIN_LEGAL_MOVES:-2}"
SUBSET_MAX_ABS_ROOT_SCORE_CP="${SUBSET_MAX_ABS_ROOT_SCORE_CP:-3000}"
SUBSET_MAX_SELECTED_REGRET_CP="${SUBSET_MAX_SELECTED_REGRET_CP:-3000}"
SUBSET_MAX_CANDIDATE_REGRET_CP="${SUBSET_MAX_CANDIDATE_REGRET_CP:-3000}"
...
echo "SUBSET_MODE=$SUBSET_MODE SUBSET_SEED=$SUBSET_SEED"
echo "SUBSET_FILTERS exclude_in_check=$SUBSET_EXCLUDE_IN_CHECK min_legal_moves=$SUBSET_MIN_LEGAL_MOVES max_abs_root_score_cp=$SUBSET_MAX_ABS_ROOT_SCORE_CP max_selected_regret_cp=$SUBSET_MAX_SELECTED_REGRET_CP max_candidate_regret_cp=$SUBSET_MAX_CANDIDATE_REGRET_CP"
...
sample_args=(
  --min-legal-moves "$SUBSET_MIN_LEGAL_MOVES"
  --max-abs-root-score-cp "$SUBSET_MAX_ABS_ROOT_SCORE_CP"
  --max-selected-regret-cp "$SUBSET_MAX_SELECTED_REGRET_CP"
  --max-candidate-regret-cp "$SUBSET_MAX_CANDIDATE_REGRET_CP"
)
if [[ "$SUBSET_EXCLUDE_IN_CHECK" == "1" ]]; then
  sample_args+=(--exclude-in-check)
fi

python3 tools/sample_jsonl.py \
  --input "$SOURCE_TRAIN" \
  --output "$RUN_DIR/train.tree.jsonl" \
  --lines "$TRAIN_LINES" \
  --seed "$SUBSET_SEED" \
  --mode "$SUBSET_MODE" \
  "${sample_args[@]}"
python3 tools/sample_jsonl.py \
  --input "$SOURCE_VALID" \
  --output "$RUN_DIR/valid.tree.jsonl" \
  --lines "$VALID_LINES" \
  --seed "$((SUBSET_SEED + 1))" \
  --mode "$SUBSET_MODE" \
  "${sample_args[@]}"
...
python3 tools/write_training_manifest.py \
  ...
  --arg SUBSET_MODE="$SUBSET_MODE" \
  --arg SUBSET_SEED="$SUBSET_SEED" \
  --arg SUBSET_EXCLUDE_IN_CHECK="$SUBSET_EXCLUDE_IN_CHECK" \
  --arg SUBSET_MIN_LEGAL_MOVES="$SUBSET_MIN_LEGAL_MOVES" \
  --arg SUBSET_MAX_ABS_ROOT_SCORE_CP="$SUBSET_MAX_ABS_ROOT_SCORE_CP" \
  --arg SUBSET_MAX_SELECTED_REGRET_CP="$SUBSET_MAX_SELECTED_REGRET_CP" \
  --arg SUBSET_MAX_CANDIDATE_REGRET_CP="$SUBSET_MAX_CANDIDATE_REGRET_CP" \
  ...
  --arg RERANK_MAX_POSITIONS="$RERANK_MAX_POSITIONS" \
  --arg RERANK_REQUIRE_MEAN_REGRET_IMPROVEMENT_CP="$RERANK_REQUIRE_MEAN_REGRET_IMPROVEMENT_CP" \
  --arg RERANK_REQUIRE_P90_REGRET_IMPROVEMENT_CP="$RERANK_REQUIRE_P90_REGRET_IMPROVEMENT_CP" \
  --arg RERANK_REQUIRE_P95_REGRET_IMPROVEMENT_CP="$RERANK_REQUIRE_P95_REGRET_IMPROVEMENT_CP" \
  --arg RERANK_REQUIRE_MATCH_RATE_IMPROVEMENT_PCT="$RERANK_REQUIRE_MATCH_RATE_IMPROVEMENT_PCT" \
  --arg KEEP_CANDIDATE_RAW="$KEEP_CANDIDATE_RAW" \
  --arg BLEND_RATIOS="$BLEND_RATIOS" \

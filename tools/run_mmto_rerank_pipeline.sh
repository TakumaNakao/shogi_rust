...
python3 tools/write_training_manifest.py \
  ...
  --run-dir "$RUN_DIR" \
  --kind mmto_rerank_pipeline \
  --arg POSITIONS="$POSITIONS" \
  --arg MAX_POSITIONS="$MAX_POSITIONS" \
  --arg VALID_PERCENT="$VALID_PERCENT" \
  --arg POSITION_CHUNK_SIZE="$POSITION_CHUNK_SIZE" \
  --arg JOBS="$JOBS" \
  --arg TEACHER_DEPTH="$TEACHER_DEPTH" \
  --arg STUDENT_DEPTH="$STUDENT_DEPTH" \
  --arg TEACHER_SCORE_TOP="$TEACHER_SCORE_TOP" \
  --arg CANDIDATE_TOP="$CANDIDATE_TOP" \
  --arg SCORE_ALL_LEGAL_FOR_VALID="$SCORE_ALL_LEGAL_FOR_VALID" \
  --arg EPOCHS="$EPOCHS" \
  ...
  --line-count positions \
  --line-count train \
  --line-count valid \
  --env RUST_FONTCONFIG_DLOPEN \
  --command bash tools/run_mmto_rerank_pipeline.sh

...
KPP重みを棋譜手だけでなく探索結果と整合させる実験は、`docs/mmto_tree_training.md` に手順をまとめています。長時間学習へ進める前に、必ず小さなscreenで `best_epoch>0`、score gate、rerank gateを確認します。

既存dumpを再利用したlistwise screen:
...
listwise微調整で改善信号が出ない場合は、強teacherでhard局面を再dumpし、teacher best と現行探索bad moveのfeedbackを使うscreenへ進みます。
...
`KEEP_SCREEN_BINARY=0` が既定なので、screenでは大きな重みファイルを残しません。候補を保持してベンチへ進めるときだけ `KEEP_SCREEN_BINARY=1` を指定してください。
